...
install_ss_panel_mod_v3(){
	...
	echo "跳过lnmp";
	echo " 正在部署环境..."
yum install -y git redhat-lsb curl gawk tar httpd-devel unzip sharutils expect
yum install wget tar gcc gcc-c++ openssl openssl-devel pcre-devel python-devel libevent automake autoconf libtool make -y
#rpm -ivh http://mirrors.mooncn.win/SSpanel/epel-release-6-8.noarch.rpm  >/dev/null 2>&1
#rpm -ivh http://mirrors.mooncn.win/SSpanel/remi-release-6.rpm  >/dev/null 2>&1
sleep 1
	echo -e "请输入数据库用户名";
	read sqluser;
	echo -e "请输入数据库密码";
...
echo "正在下载lnmp环境源...";
#注释掉phpmyadmin的安装
#wget ${web}${MirrorHost}/${sqrd}/phpmyadmin.zip 
...
mysql -uroot -p$sqlpass -e"CREATE DATABASE sspanel;" 
	fi
#	cd /var/www/
#	cp -r html/phpmyadmin/ .
	...
	wget -N -P  /etc/nginx/ --no-check-certificate https://raw.githubusercontent.com/mooncn/ssenvironment/master/nginx.conf
	service nginx restart
	IPAddress=`wget http://members.3322.org/dyndns/getip -O - -q ; echo`;
	sed -i "s#103.74.192.11#${IPAddress}#" /var/www/html/sql/sspanel.sql
	mysql -u$sqluser -p$sqlpass sspanel < /var/www/html/sql/sspanel.sql
	cd /var/www/html
	...
	echo '30 22 * * * php /var/www/html/xcat sendDiaryMail' >> /var/spool/cron/root
	echo '0 0 * * * php /var/www/html/xcat dailyjob' >> /var/spool/cron/root
	echo '*/1 * * * * php /var/www/html/xcat checkjob' >> /var/spool/cron/root
...
install_panel_and_node(){
  ...
  sed -i '$a [program:ssr]\ncommand = python /root/shadowsocks/server.py\nuser = root\nautostart = true\nautorestart = true' /etc/supervisord.conf
	...
	echo "# phpmyadmin:已屏蔽 用户名:root 密码为:sqlpass  #"
	echo "# 安装完成，节点即将重启使配置生效                           #"
	echo "# Github: https://github.com/mooncn/ssenvironment    #"
	echo "#############################################################"
	reboot now
}
install_speeder(){
...
echo "# 3  Speed up One click Install                             #"
echo "#############################################################"
echo
num=$1
if [ "${num}" == "1" ]; then
